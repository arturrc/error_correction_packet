...
// Typical use: fb_trie_rethread_5 -pop m -bc n -cpu o file
// Can also accept cin: cat file | fb_trie_rethread_5 -pop m -bc n -cpu o
		...
		for(auto it2 = pairs.begin();it2 != pairs.end();++it2){
			...
			if(confirmed_reads[pop][it2->second] == 0){
				...
				for(auto it3 = matches.begin();it3 != matches.end();++it3){
					...
					// Check if the matched barcode is less than 1/err_ratio % of the current count. Errors should not be a significant portion of the cluster.
					//it2->second is the master seed.
					//matched_bc is the one we are testing
					if(confirmed_reads[pop][matched_bc]*err_ratio > confirmed_reads[pop][it2->second] || matched_bc == it2->second){
						continue;
					} 
...
int main(int argc, char * argv[]) {
	...
	// Parse arguments
	// Typical use: fb_trie_rethread_3 -pop n -bc m file
	// Can also accept cin: cat file | fb_trie_rethread_3 -pop n -bc m
	...
	if(argc < 5){
		cerr << "fb_trie_rethread_5 [-cpu i] [-err i] [-err_ratio i] [-min_count i] -pop n -bc m file" << endl;
		cerr << "cat file | fb_trie_rethread_5 [-cpu i] [-err i] [-err_ratio i] [-min_count i] -pop n -bc m" << endl;
		return 1;
	}
	else{
		for(int i = 1;i < argc;++i){
			...
			if(string(argv[i]) == "-err"){
				err_max = atoi(argv[i+1]);
				++i;
				continue;
			}
			if(string(argv[i]) == "-min_count"){
				min_count = atoi(argv[i+1]);
				++i;
				continue;
			}
			if(string(argv[i]) == "-cpu"){
				cpu_count = atoi(argv[i+1]);
				omp_set_num_threads(cpu_count);
				++i;
				continue;
			}
			if(string(argv[i]) == "-err_ratio"){
				err_ratio = atof(argv[i+1]);
				++i;
				continue;
	...
	for(string read; getline( *in, read); ){
		...
		// Once you reach min_count, then move the reads from reads to confirmed_reads. Add the confirmed to the trie.
		if(reads[pop][bc].size() == (min_count-1)){
			...
			confirmed_reads[pop][bc] = min_count;
			number_of_reads_temporary_confirmed += min_count;
